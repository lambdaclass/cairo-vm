...
func sha256{range_check_ptr, sha256_ptr : felt*}(input : felt*, n_bytes : felt) -> (output : felt*):
    ...
    %{
        ...
        print(memory.get_range(ids.sha256_start, _sha256_input_chunk_size_felts))
...
func main{output_ptr: felt*, range_check_ptr, bitwise_ptr : BitwiseBuiltin*}():
    ...
    %{
        ...
        mem = memory.__dict__["_ValidatedMemoryDict__memory"]
        for k,v in mem.items():
            if RelocatableValue.to_tuple(k)[0] == 8:
                print(k, "->", v)
    %}
