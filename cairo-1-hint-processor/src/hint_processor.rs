...
impl HintProcessor for Cairo1HintProcessor {
    ...
    fn compile_hint(
        ...
        let data = hint_code.parse().ok().and_then(|x| self.hints.get(&x)).ok_or(VirtualMachineError::CompileHintFail(format!("No hint found for pc {}. Cairo1HintProccesor can only be used when running CasmContractClass", hint_code)))?;
        Ok(any_box!(data))
    }
