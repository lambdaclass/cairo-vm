...
pub trait HintProcessorLogic {
    ...
    // Note: if the `load_program` feature is activated the method used by the vm to execute hints is `execute_hint_extensive`, which's default implementation calls this method.
    fn execute_hint(
        &mut self,
