...
fn felt_to_scaled_decimal(f: &Felt252) -> Option<Decimal> {
    Some(Decimal::from_str_radix(&f.to_signed_felt().to_string(), 10).ok()? * *DECIMAL_ADJUSTMENT)
}
    ...
    fn read_from_memory(memory: &Memory, read_ptr: Relocatable) -> Option<Self> {
        Some(Position {
            ...
            amount: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 1_i32).ok()?).ok()?.as_ref(),
            )?,
            cost: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 2_i32).ok()?).ok()?.as_ref(),
            )?,
            cached_funding: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 3_i32).ok()?).ok()?.as_ref(),
            )?,
        })
    }
}
    ...
    fn read_from_memory(memory: &Memory, read_ptr: Relocatable) -> Option<Self> {
        Some(MarginParams {
            ...
            imf_base: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 4_i32).ok()?).ok()?.as_ref(),
            )?,
            imf_factor: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 5_i32).ok()?).ok()?.as_ref(),
            )?,
            mmf_factor: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 6_i32).ok()?).ok()?.as_ref(),
            )?,
            imf_shift: felt_to_scaled_decimal(
                memory.get_integer((read_ptr + 7_i32).ok()?).ok()?.as_ref(),
            )?,
        })
    }
...
pub fn excess_balance_hint(
    ...
    for position in balances {
        ...
        let margin_fraction = if margin_check_type.as_ref() == margin_check_initial {
