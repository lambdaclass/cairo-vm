...
mod tests {
    ...
    fn element_not_found_search() {
        ...
        assert_matches!(
            ...
            Err(HintError::NoValueForKeyFindElement(bx)) if *bx == Felt::new(7)
        );
    }
    ...
    fn find_elm_zero_elm_size() {
        ...
        assert_matches!(
            run_hint!(vm, ids_data, hint_code::FIND_ELEMENT),
            Err(HintError::ValueOutOfRange(bx)) if *bx == Felt::ZERO
        );
    }
    ...
    fn find_elm_negative_elm_size() {
        ...
        assert_matches!(
            run_hint!(vm, ids_data, hint_code::FIND_ELEMENT),
            Err(HintError::ValueOutOfRange(bx)) if *bx == Felt::new(-1)
        );
    }
    ...
    fn find_elm_negative_n_elms() {
        ...
        assert_matches!(
            run_hint!(vm, ids_data, hint_code::FIND_ELEMENT),
            Err(HintError::Math(MathError::Felt252ToI32Conversion(bx))) if *bx == Felt::new(-1)
        );
    }
    ...
    fn find_elm_n_elms_gt_max_size() {
        ...
        assert_matches!(
            run_hint!(vm, ids_data, hint_code::FIND_ELEMENT, &mut exec_scopes),
            Err(HintError::FindElemMaxSize(bx)) if *bx == (Felt::ONE, Felt::new(2))
        );
    }
