        ...
        match instruction.opcode {
            ...
            Opcode::AssertEq => match (&instruction.res, dst, op1, &instruction.opcode_extension) {
                ...
                (
                    ...
                    Some(MaybeRelocatable::Int(num_dst)),
                    Some(MaybeRelocatable::Int(num_op1)),
                    OpcodeExtension::Stone | OpcodeExtension::QM31Operation,
                ) if !num_op1.is_zero() => {
                    let op0_val =
    ...
    fn deduce_op1(
        ...
        if let Opcode::AssertEq = instruction.opcode {
            match (instruction.res, instruction.opcode_extension) {
                ...
                (Res::Op1, OpcodeExtension::QM31Operation) => {
                    if let (
                        Some(MaybeRelocatable::Int(num_dst)),
                ...
                (Res::Mul, OpcodeExtension::Stone | OpcodeExtension::QM31Operation) => {
                    match (dst, op0) {
                        (
                            Some(MaybeRelocatable::Int(num_dst)),
                            Some(MaybeRelocatable::Int(num_op0)),
...
mod tests {
    ...
    fn compute_res_qm31_add_relocatable_values() {
        ...
        assert_matches!(
            vm.compute_res(&instruction, &op0, &op1),
            Err(VirtualMachineError::ComputeResRelocatableQM31Add(bx)) if *bx == (op0, op1)
        );
    }
