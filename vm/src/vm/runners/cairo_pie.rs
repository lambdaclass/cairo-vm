...
pub struct CairoPieMetadata {
    ...
    /// `simulated_builtins` are builtins that are being verified in the executed cairo code (so
    /// proving them only involves proving cairo steps) rather than being outputted as their own
    /// segment and proven later using dedicated AIRs (see example implementation of this mechanism
    /// in `simple_bootloader.cairo`).
