...
impl CairoRunner {
    ...
    /// only used for providing the needed data for the PIE creation at the end of the run
    /// (which is the same for running PIEs and programs).
...
mod tests {
    ...
    */
    ...
    fn test_load_simulated_builtin_runners_failure() {
        ...
        assert_matches!(
            result,
            Err(CairoRunError::UnsupportedSimulatedBuiltin(
                BuiltinName::pedersen
            ))
        );
    }
