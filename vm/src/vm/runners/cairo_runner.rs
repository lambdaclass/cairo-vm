    ...
    pub fn get_info_for_prover_input(&self) -> Result<ProverInfo, RunnerError> {
        Ok(ProverInfo {
            ...
            .map(|(segment, offset_page)| {
                ...
                (*segment, offsets) // Now we pass a concrete `Vec<usize>` instead of `[usize]`
            })
            .collect(),
            builtins_segments: self.get_builtin_segment_info_for_pie()?.into_iter().map(|(name, info)| (name, info.index)).collect(),
        })
    }
}
...
mod tests {
    ...
    */
    ...
    fn get_prover_info() {
        ...
        assert_eq!(prover_info.builtins_segments, HashMap::from([(BuiltinName::ecdsa, 2)]));
    }
}
