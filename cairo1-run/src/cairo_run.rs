...
pub fn cairo_run_program(
    ...
    let program_registry_info =
        ProgramRegistryInfo::new(sierra_program).map_err(|_| VirtualMachineError::Unexpected)?;
    let metadata = calc_metadata_ap_change_only(sierra_program, &program_registry_info)
...
mod tests {
    ...
    fn compile_to_sierra(filename: &str) -> SierraProgram {
        ...
        let mut db = RootDatabase::builder()
            ...
            .build()
            .unwrap();
        let main_crate_inputs = setup_project(&mut db, Path::new(filename)).unwrap();
