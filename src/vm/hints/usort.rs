...
pub fn usort_body(
    ...
    for (i, repetition_amount) in multiplicities.into_iter().enumerate() {
        vm_proxy.memory.insert_integer(
    ...
    insert_relocatable_from_var_name("output", output_base, ids, vm_proxy, hint_ap_tracking)?;
    insert_relocatable_from_var_name(
        "multiplicities",
        multiplicities_base,
        ids,
...
pub fn verify_multiplicity_body(
    ...
    insert_integer_from_var_name("next_item_index", pos_diff, ids, vm_proxy, hint_ap_tracking)?;
