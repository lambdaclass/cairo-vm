...
impl KeccakBuiltinRunner {
    ...
    pub fn deduce_memory_cell(
        ...
        let present_in_memory = vec![];

        for i in 0..self.n_input_cells {
            present_in_memory.push(memory.data.contains(&first_input_addr + i as usize))
        }

        if !present_in_memory.iter().all(|&x| x == true) {
            return Ok(None);
        }

        for (i, bits) in (0..self.state_rep.len()).zip(0..self.state_rep.len()) {
            let Ok(Some(value)) = memory.get(&(first_input_addr.into() + i));

            let val = match value.into_owned() {
            ...
            for i in 0..self.n_input_cells {
                Ok(Some(value)) = memory.get(&(first_input_addr.into() + i));

                input_felts.push(value)
            }
        }
        Ok(None)
    }
...
mod tests {
    ...
    fn point_is_on_curve_a() {
        ...
        assert!(KeccakBuiltinRunner::point_on_curve(
    ...
    fn compute_ec_op_impl_valid_a() {
        ...
        let result =
            KeccakBuiltinRunner::ec_op_impl(partial_sum, doubled_point, &m, &alpha, &prime, height);
        assert_eq!(
            result,
            Ok((
                bigint_str!(
    ...
    fn compute_ec_op_invalid_same_x_coordinate() {
        ...
        let result = KeccakBuiltinRunner::ec_op_impl(
            partial_sum.clone(),
            doubled_point.clone(),
            &m,
            &alpha,
    ...
    fn deduce_memory_cell_ec_op_for_preset_memory_m_over_scalar_limit() {
        ...
        assert_eq!(
            ...
            Err(RunnerError::EcOpBuiltinScalarLimit(
                builtin.keccak_builtin.scalar_limit.clone()
            ))
        );
    }
