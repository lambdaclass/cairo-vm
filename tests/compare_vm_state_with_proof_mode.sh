...
tests_path="../cairo_programs/proof_programs"
exit_code=0
trace=false
memory=false
passed_tests=0
...
for file in $(ls $tests_path | grep .cairo$ | sed -E 's/\.cairo$//'); do
    ...
    if $trace; then
        if ! diff -q $path_file.trace.proof $path_file.rs.trace.proof; then
    ...
    if $memory; then
        if ! ./memory_comparator.py $path_file.memory.proof $path_file.rs.memory.proof; then
